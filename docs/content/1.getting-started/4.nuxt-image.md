...
::callout{type="info"}
Think of it this way: thumbnails in the grid are `'thumb'` images and don't need to be full resolution. When a photo opens full-screen in the lightbox, that's a `'slide'` image and needs to be sharp at the full viewport size. The adapter knows which situation it's in, so it can request the right size from your image CDN.
::
...
When `@nuxt/image` is not installed (or `provider` is `'native'`), nuxt-photo uses plain `<img>` tags with the `src` and `srcset` from your `PhotoItem` data directly. You can provide your own `srcset` strings on each photo:
...
See the [Image Adapters guide](/guides/image-adapters) for more details.
