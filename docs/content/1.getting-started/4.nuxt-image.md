...
nuxt-photo resolves every rendered image through an adapter. When `@nuxt/image` is present, the module provides a Nuxt Image adapter automatically.
...
`'thumb'` is for grid cells, `'slide'` is for the fullscreen viewer, and `'preload'` is reserved for ahead-of-time fetching.
...
When `@nuxt/image` is not installed, nuxt-photo falls back to the `src`, `thumbSrc`, and `srcset` already present on each `PhotoItem`:
...
See the [Image Adapters guide](/guides/image-adapters) for adapter design, custom CDNs, and `PhotoAdapter` vs `ImageAdapter`.
