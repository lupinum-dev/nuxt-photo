...
The `responsive()` helper creates a function that picks the right value based on the album's container width. The keys are minimum widths in pixels — so `{ 0: 2, 768: 3, 1200: 4 }` means: 2 columns by default, 3 at ≥768px, and 4 at ≥1200px. See the [responsive guide](/guides/responsive) for more.
