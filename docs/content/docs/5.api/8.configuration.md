...
```ts
nuxtPhoto: {
  autoImports: false // manually import from '@nuxt-photo/nuxt/app'
}
```

Turn this off only if you want tight control over your imports or you are importing the composables manually. Nuxt apps should use the supported `@nuxt-photo/nuxt/app` facade. If app code deliberately imports from `@nuxt-photo/vue` directly, list `@nuxt-photo/vue` in your app dependencies.
...
:read-more{to="/docs/api/css" title="CSS reference and custom theming"}
...
`ImageAdapterKey` is exported from `@nuxt-photo/vue`, so direct app imports need `@nuxt-photo/vue` as an explicit dependency.
...
:read-more{to="/docs/concepts/image-pipeline" title="How image providers work"}
...
- `vite.server.fs.allow` receives the linked `@nuxt-photo/nuxt` and
  `@nuxt-photo/vue` package roots so local workspace development can serve files
  from both packages.
...
:read-more{to="/docs/concepts/photo-model" title="The PhotoItem type"}
