...
The `lightbox` capability is setup-time only. Remount the component with a new
`key` to add, remove, or replace its lightbox. The `transition` option remains
reactive.
...
Use the `slide` slot when the photo's lightbox view needs to differ from its
thumbnail, such as a before-and-after comparison or a credit overlay.
