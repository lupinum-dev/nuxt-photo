...
<template>
  <LightboxProvider :photos="photos">
    ...
    >
      ...
      >
        <button :aria-label="labels.close" @click="close">{{ labels.close }}</button>
        <span aria-live="polite">{{ labels.counter(activeIndex + 1, count) }}</span>
        <div class="flex gap-2">
          <button :aria-label="labels.previous" @click="prev">{{ labels.previous }}</button>
          <button :aria-label="labels.next" @click="next">{{ labels.next }}</button>
        </div>
      </LightboxControls>
