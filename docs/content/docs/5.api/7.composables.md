...
Returns the complete active label set. It merges labels provided through
`PhotoDefaultsKey` or `nuxtPhoto.labels` with the built-in English defaults.
Use it when a custom lightbox built from primitives renders its own controls.
