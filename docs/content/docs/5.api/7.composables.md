...
description: Read lightbox state, create providers, localize labels, resolve responsive values, and measure containers.
---
...
Reads the same resolved labels used by recipe components and primitives. Use it
when a custom lightbox interface must stay consistent with the library UI.
...
Provides reactive Vue labels. Indexed messages are functions, so they can use
the current index, count, grammar, and locale.
...
providePhotoLabels(
  computed(() => ({
    ...
    viewPhoto: (index) => `Open photo ${index}`,
  })),
)
```

Nuxt AppConfig uses serializable string templates instead; see
[Nuxt configuration](/docs/api/configuration).
