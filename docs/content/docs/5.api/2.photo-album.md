...
| `sizes`                 | `string \| { size: string; sizes?: ... }`         | -                        | Native string or layout-aware rows `<img sizes>` hint.          |
...
remove, or replace it. Photos, layout options, and transition configuration stay
reactive.

## Programmatic control

`PhotoAlbum` exposes the shared `LightboxHandle` through a template ref:
...
When an album is inside `PhotoGroup`, its indexes stay local to the album. The
album resolves the selected photo ID before it delegates to the group.
