...
Interactive lightbox children opt back into pointer events inside the full-screen UI layer. Keep that pattern if you replace the included markup: backdrop clicks should reach `.np-lightbox__backdrop`, while controls, captions, and media stay clickable.
