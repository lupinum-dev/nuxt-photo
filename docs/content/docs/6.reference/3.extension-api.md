...
| `PhotoGroup` `trigger` | `@nuxt-photo/recipes` | Custom thumbnail layouts that keep the recipe lightbox               | Available in the custom-layout slot              |
| Recipe slots           | `@nuxt-photo/recipes` | Thumbnails, built-in lightbox slots, and per-photo slide overrides   | Use before rebuilding the lightbox               |
