...
`@nuxt-photo/core`. Nuxt apps can import the common public types from
`@nuxt-photo/nuxt`; plain Vue apps using recipes can import them from
`@nuxt-photo/recipes`.
...
```ts
type PhotoMapper<T = unknown> = (item: T) => PhotoItem
```

Transforms an external data shape (e.g. a CMS response) into a `PhotoItem`. Pass via the `:itemMapper` prop of `<PhotoAlbum>`, `<PhotoGroup>`, or `<PhotoCarousel>`.
...
```ts
type GestureMode = 'idle' | 'slide' | 'pan' | 'pinch' | 'close'
```

Current gesture phase reported by the viewer runtime. `pinch` is emitted for two-pointer zoom gestures, `pan` for dragging a zoomed slide, `slide` for carousel swipes, and `close` for drag-to-dismiss.
