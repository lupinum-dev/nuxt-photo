---
title: Layers
description: Five supported entry points. Pick the highest one that does the job.
---
...
```text
 ...
 │ 3. <PhotoGroup v-slot>                              │
 ├─────────────────────────────────────────────────────┤
 │ 4. Primitives: <LightboxProvider>, <LightboxRoot>, … │
 ├─────────────────────────────────────────────────────┤
...
The default path. Four components, all batteries included.
...
<LightboxProvider :photos="photos">
  ...
  >
    <LightboxOverlay class="backdrop" />
...
`<LightboxProvider :photos>` creates the context for triggers and lightbox primitives. `<LightboxRoot>` consumes that context, teleports the lightbox to `<body>`, renders only while open, and owns the focus trap.
...
At the bottom are the composables that power everything above. They expose Vue state, actions, DOM refs, and gesture handlers.
...
- You want to mirror lightbox state into a Pinia store, a URL, or an external event source.
- You are building your own Vue shell around the lightbox runtime.
