...
If your API returns something else — Unsplash, Contentful, Sanity — use a `PhotoMapper` instead of manually mapping.
...
Pass it to `<PhotoAlbum :item-mapper="fromUnsplash" :photos="apiResponse" />` and Nuxt Photo will transform each item before rendering.
