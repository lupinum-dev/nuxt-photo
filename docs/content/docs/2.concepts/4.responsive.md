...
you use `@nuxt-photo/vue` or `@nuxt-photo/recipes` directly in a plain Vue app.
...
- `<PhotoAlbum>` rows layout — `layout.targetRowHeight` or the `targetRowHeight` shorthand prop
- `<PhotoAlbum>` columns/masonry layout — `layout.columns` or the `columns` shorthand prop
