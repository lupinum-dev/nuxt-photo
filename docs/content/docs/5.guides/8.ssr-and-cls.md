...
The layout runs on the server at 1200px. For rows, columns, and masonry this lets the server render the same grouped layout the client starts with.
...
`<PhotoCarousel>` uses CSS-only sizing while Embla hydrates the scroll behavior on mount. There is no CLS risk from the layout itself. Make sure photo `width` and `height` match your `slideAspect`.
