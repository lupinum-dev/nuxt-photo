...
<template>
  ...
  <LightboxRoot class="fixed inset-0 z-50" role="dialog" aria-modal="true">
...
- `<LightboxPortal>` outside the open lightbox root — the ghost image paints in the wrong stacking context.
...
For deeper customization, reach into the full extension API when you need to replace engine behavior instead of only changing the Vue layout.
