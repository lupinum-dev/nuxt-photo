...
`trigger()` already wires the source thumbnail ref for FLIP transitions. In a custom layout `<PhotoGroup>` slot you do not get a public `hiddenIndex` or `hiddenThumbIndex` value, so the default lightbox handles the source-thumb hiding for you.
