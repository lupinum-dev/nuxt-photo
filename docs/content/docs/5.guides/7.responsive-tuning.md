...
Downside: no breakpoint metadata for client snapping (see the note in [Responsive](/docs/concepts/responsive#ssr-and-breakpoints)). Pair with an explicit `:breakpoints` prop if snapped measurement matters.
