...
description: Choose rows, columns, or masonry and resolve parameters from the gallery container.
---

Spacing, padding, columns, and row height can all change per container width. Instead of media queries or `v-if`, Nuxt Photo accepts a **responsive parameter** wherever a number is expected - a plain value, a breakpoint map, or a function.

::responsive-lab
::

In Nuxt apps, `responsive()` is auto-imported by the module. Import it from
`@lupinum/vue-photo` only in a plain Vue app.
...
```vue
<PhotoAlbum :photos="photos" :spacing="responsive({ 0: 4, 640: 8, 1024: 12 })" />
```
...
```vue
<PhotoAlbum :photos="photos" :spacing="(w) => (w < 600 ? 4 : w < 1200 ? 8 : 12)" />
```
...
Static props such as `layout.type`, `photos`, and `lightbox` do not respond to container width.
...
- **SSR** needs a deterministic layout before JavaScript runs. Media queries cannot guarantee this layout without an assumed viewport.

The responsive parameter gives JS and CSS the same source of truth.

## SSR and `breakpoints`

When you use `responsive()`, its positive breakpoint keys are extracted automatically and used as the client snap set. Breakpoint `0` is the base value, not a snap width.

Combine with `defaultContainerWidth` when the server should render a deterministic JS layout:
...
The server renders with `spacing=12` (because 1200 ≥ 1024), then the client re-resolves at its real measured width after mount. If your assumed width matches the common viewport, users see no flicker.

- [SSR, CLS, and defaultContainerWidth](/docs/concepts/ssr-and-layout-stability)
...
`responsive()` cannot switch layout _types_. Use it for numeric values. To change the layout type, conditionally render the component:
