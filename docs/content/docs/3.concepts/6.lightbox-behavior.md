...
1. **The thumbnail is out of view.** Scroll to it, then open - `'auto'` can FLIP once enough of the thumbnail is visible. This is correct behavior: animating from a rect the user can't see looks wrong.
...
:::read-more{to="/docs/api/composables" title="useLightboxProvider reference"}
:::

:::read-more{to="/docs/api/types" title="TransitionMode reference"}
:::
