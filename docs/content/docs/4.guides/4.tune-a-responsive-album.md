...
Masonry uses the same responsive `columns` parameter, but it fills the shortest column first instead of using row order. Users scan this layout and do not read it from top to bottom.
...
Match item `padding` to the frame treatment, not to the spacing. These values control different visual cues.
...
```vue
<PhotoAlbum :photos="photos" :spacing="(w) => Math.round(Math.max(4, Math.min(16, w / 80)))" />
```

At 320px: 4px spacing. At 1280px: 16px spacing. Linear between.

Downside: no breakpoint metadata for client snapping (see the note in [Responsive](/docs/concepts/layouts-and-responsiveness#ssr-and-breakpoints)). Pair with an explicit `:breakpoints` prop if snapped measurement matters.
...
The responsive prop uses the measured width of the `<PhotoAlbum>` **container**. This behavior is useful when albums are in sidebars or cards with independent maximum widths.
