...
- `open(photoOrIndex)` — open the lightbox at an index or matching `PhotoItem`.
...
All state values are refs or computed refs; unwrap them in templates as usual (`lightbox.activeIndex.value`).
