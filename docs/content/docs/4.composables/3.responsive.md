...
In Nuxt apps, the module auto-imports `responsive()`. In plain Vue apps, import
it from the package you are already using:

```ts
import { responsive } from '@nuxt-photo/recipes'
// or: import { responsive } from '@nuxt-photo/vue'
```
...
A key of `0` is the clearest fallback when the container is narrower than every other breakpoint. If you omit it, the resolver falls back to the lowest key you did provide.
...
`responsive()` quietly tags the returned function with its positive breakpoint keys, so `<PhotoAlbum>` can use them as the client snap set. With `defaultContainerWidth`, the server also resolves the same responsive value for the assumed width:
...
…runs the server layout with `spacing=12` (because 1024 ≥ 1024). The client then snaps measured widths to `[640, 1024]` after mount.

Inline functions lose this metadata. If you want client snapping with an inline function, pass an explicit `:breakpoints` prop:
...
Negative or infinite keys are ignored when the breakpoint set is computed. Breakpoint `0` is the base value and is not included in the snap set.
