...
During module setup, Nuxt Photo mutates two Vite dev settings:
...
- `vite.server.fs.allow` receives the linked `@nuxt-photo/nuxt` and
  ...
  from both packages.
- `vite.optimizeDeps.include` receives Embla packages used by
  `<PhotoCarousel>`, which avoids repeated dependency discovery during Nuxt dev
  server startup.

These entries are appended only when missing. Existing app configuration remains
in place.
