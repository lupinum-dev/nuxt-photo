...
Nuxt Photo is a single Nuxt module. One install, one entry in `modules`, and its components plus prefixed helpers are auto-imported.
...
That's it. `<Photo>`, `<PhotoAlbum>`, `<PhotoGroup>`, `<PhotoCarousel>`, plus helpers like `useNuxtPhotoLightbox`, are now available anywhere in your app.
...
You can import advanced helpers from those sibling packages when you need to, but the default auto-imports cover the common Nuxt path.
