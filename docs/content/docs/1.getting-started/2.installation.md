...
Nuxt Photo is a single Nuxt module. One install, one entry in `modules`, and its components plus helpers are auto-imported.
...
That's it. `<Photo>`, `<PhotoAlbum>`, `<PhotoGroup>`, `<PhotoCarousel>`, plus helpers like `useLightbox`, are now available anywhere in your app.
