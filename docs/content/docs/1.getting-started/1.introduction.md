...
The lightbox supports pinch-to-zoom, pan, swipe, and spring-physics FLIP transitions. The module auto-imports components and helpers by default, detects `@nuxt/image`, and routes thumbnails and slides through it when it is installed.
...
You get a justified-rows layout, a lightbox with gesture support, and server-rendered markup without extra config.
...
- SSR-friendly rows, plus deterministic columns/masonry SSR when you provide `defaultContainerWidth`.
- Lightbox primitives (`LightboxRoot`, `LightboxViewport`, `PhotoTrigger`, …) for custom lightbox structure.
