...
3. **Inside `<PhotoGroup>`** — auto-registers with the group's shared lightbox when the group lightbox is enabled. Clicking opens that shared lightbox. If the group has `:lightbox="false"`, the photo renders plain. The `lightbox` prop is ignored inside a group; use `lightboxIgnore` to opt out of an enabled group.
