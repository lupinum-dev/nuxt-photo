---
title: PhotoImage
description: Adapter-backed `<img>` wrapper for custom thumbnails and slides.
---
...
| `context`      | `'thumb' \| 'slide'` | `'thumb'`               | Rendering context — adapters may return different URLs per context. |
...
`<PhotoImage>` asks the current adapter for this photo's `src`, `srcset`, and `sizes`, then renders a plain `<img>` with those attributes. With `@nuxt/image`, the adapter computes Nuxt Image URLs; `<PhotoImage>` still renders the final `<img>` itself.
