...
| `itemMapper`            | `PhotoMapper`                                     | —                        | Transforms each `photos` entry into a `PhotoItem` (for CMS data). |
| `validation`            | `'throw' \| 'drop' \| 'warn'`                     | `'throw'`                | Invalid photo policy. Use `drop`/`warn` only for dynamic content. |
...
| `targetRowHeight`       | `ResponsiveParameter<number>`                     | `300`                    | Rows-layout target height shorthand.                              |
| `columns`               | `ResponsiveParameter<number>`                     | `3`                      | Columns/masonry column-count shorthand.                           |
...
### Feeding CMS data with an `itemMapper`
...
const fromContentful: PhotoMapper<ContentfulAsset> = (item) => ({
  id: item.sys.id,
...
```vue
<PhotoAlbum :photos="contentfulAssets" :item-mapper="fromContentful" />
```
