...
The theme is built on CSS custom properties. Override them when `css: 'all'` is active and you want a new look without replacing the structure:
...
Then style lightbox elements with your own utilities via `itemClass` and `imgClass`, or build a fully custom lightbox with [primitives](/components/primitives).
