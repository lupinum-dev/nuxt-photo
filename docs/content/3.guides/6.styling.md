...
Then style lightbox elements using Tailwind classes via the `itemClass` and `imgClass` props on `PhotoAlbum`, or build a fully custom lightbox with [primitives](/components/primitives) and apply Tailwind classes directly.
