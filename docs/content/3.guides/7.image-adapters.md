...
The adapter receives the photo plus a context: `'thumb'`, `'slide'`, or `'preload'`.

## Native Adapter (Default)

When no custom adapter is configured, nuxt-photo uses the native adapter. It returns the photo's `src`, `thumbSrc`, and `srcset` directly:
...
A `PhotoAdapter` is different from an `ImageAdapter`. It transforms **external data shapes** into `PhotoItem` objects:
