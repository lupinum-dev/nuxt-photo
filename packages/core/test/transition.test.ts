...
describe('transition planning', () => {
  ...
  it('uses transition mode rules for FLIP eligibility', () => {
    ...
    expect(
      shouldUseFlip(rect({ left: 0, top: 0, width: 300, height: 200 }), {
        mode: 'none',
        autoThreshold: 0.55,
      }),
    ).toBe(false)
    ...
    expect(
      shouldUseFlip(rect({ left: -220, top: 100, width: 400, height: 400 }), {
        mode: 'auto',
        autoThreshold: 0.55,
      }),
    ).toBe(false)
    expect(
      shouldUseFlip(rect({ left: 40, top: 40, width: 300, height: 220 }), {
        mode: 'auto',
        autoThreshold: 0.55,
      }),
    ).toBe(true)
