...
| Lightbox opens at wrong frame                            | Thumbnail dimensions or refs do not match the rendered image                           | Fix `PhotoItem` dimensions; use `PhotoTrigger` for custom layouts; pass through `hidden` in custom thumbnail slots. |
...
| Carousel autoplay behaves oddly                          | Interaction or hover behavior does not match the selected library-owned options        | Check `delayMs`, `stopOnInteraction`, and `stopOnMouseEnter`.                                                       |
...
Recipe components validate photo data before layout. Invalid photos throw in every
environment by default. Collection recipes drop entries only when
`validation="drop"` is explicitly selected.
